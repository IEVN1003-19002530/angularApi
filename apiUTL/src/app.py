...
def listar_alumnos():
    try:
        ...
        for fila in datos:
            alumno = {"matricula":fila [0], "nombre":fila[1], "apaterno":fila[2], "amaterno":[3], "correo":fila[4]}
            alumnos.append(alumno)
        return jsonify({'alumnos':alumnos, 'mensaje':'Alumnos listados', "exito":True})

    except Exception as ex:
        return jsonify({"mensaje": "Error de Base de Datos xd:{}".format(ex),'exito':False}), 500
